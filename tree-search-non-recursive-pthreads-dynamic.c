...
void* TSP(void *argument){
	...
	int active = 1;
	while (threads_working || 0<=stack->top){
		if (s->top<0){
			if (active){
				active--;
				pthread_mutex_lock(&not_working);
				threads_working--;
				pthread_mutex_unlock(&not_working);
			}
		...
		while (0<=s->top){
			if (1<=s->top && threads_working<thread_count){
				//printf("threads working: %d\n",threads_working);
				pthread_mutex_lock(&not_working);
				while (stack->top<s->top){
					push(stack, pop(s));
				}	
				split_stack++;
				pthread_mutex_unlock(&not_working);
...
int main(int argc, char **argv){
	...
	if (pthread_mutex_init(&not_working, NULL) != 0){
        printf("\n mutex init failed\n");
        return 1;
    }
